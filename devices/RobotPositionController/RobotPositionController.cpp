    ...
    for (const auto& controlBoard : controlBoards) {
        ...
        if (pImpl->controlMode == "position") {
            ...
            for (unsigned i = 0; i < pImpl->nJointsVector.at(boardCount); i++) {
                ...
                pImpl->iPosControl->setRefSpeed(i, 70);
            }
...
//                if (pImpl->controlMode == "position") {
//                    pImpl->iPosControl->positionMove(controlBoardJointIndex, pImpl->jointPositionsVector.at(humanStateJointIndex)*(180/M_PI));
//                    yInfo () << LogPrefix << "Joint name : " << pImpl->jointNameListFromControlBoards.at(controlBoardJointIndex) << " , Joint angle : " << pImpl->jointPositionsVector.at(humanStateJointIndex)*(180/M_PI);
//                }
